...
payloads that land here are ``unsupported_log``, never ``WarAttack``.
"""
...
__all__ = ["WARS_PARSER_NAME", "WARS_PARSER_VERSION", "WarsParser"]

WARS_PARSER_NAME = "wars"
WARS_PARSER_VERSION = "1"

_LOG_ATTACK = "war_attack_log"
_LOG_MISSED = "war_missed_attacks_log"
_LOG_EMBED = "war_embed_log"
...
class WarsParser:
    ...
    def parse_channel(self, messages: Iterable[Mapping[str, Any]]) -> ParseOutcome:
        ...
        missed-attacks message posted at ``warEnded``.
        """
        collected = list(messages)
        outcome = parse_all(self, collected)
        return self._assign_war_keys(outcome, collected)
    ...
    def _parse_embed(
        ...
        *,
        ...
        if regular is not None:
            # Supporting context for ``parse_channel``; not a player event.
            return ParseOutcome()
        return _unknown(
            _unknown_war_log_type(embed),
            message_id=message_id,
            channel_id=channel_id,
        )
    ...
    def _missed_events(self, missed: Any, source: SourceMetadata) -> ParseOutcome:
        ...
        key: str | None = None
        month: str | None = None
        if missed.clan_tag and missed.opponent_tag:
    ...
    def _assign_war_keys(
        ...
        for message in messages:
            ...
            if ctx is not None:
                window = window_for_regular_embed(ctx)
                if window is not None:
                    embed_windows.append(window)
                continue
            missed = parse_missed_embed(embed)
            if missed is None or missed.is_cwl:
                continue
            ended_at = parse_timestamp(message.get("timestamp"))
            if ended_at is None:
                continue
            window = window_for_missed(missed, ended_at, for_cwl=False)
            if window is not None:
        ...
        for event in outcome.events:
            if isinstance(event, WarAttack):
                ...
                continue
            if isinstance(event, WarMissedAttacks):
                window = window_matching_miss(event.occurred_at, event.opponent_tag, embed_windows)
                if window is None:
                    events.append(event)
                    continue
                events.append(
...
def _unknown_war_log_type(embed: Mapping[str, Any]) -> str:
    ...
    if isinstance(fields, list):
        field_names = " ".join(
            as_str(field.get("name")) or "" for field in fields if isinstance(field, Mapping)
        )
    if "Missed Attacks" in description or "Missed Attacks" in field_names:
        return _LOG_MISSED
    return "war_log"
